...
local sliderXml = [[
    ...
    autoWidth="false" autoHeight="false" 
    flowDirection="top_to_bottom">
        <Button mouseClick="{TEST_UI_ID#close}" text="Close"/>
        <ScaledSlider min="10" max="20" current="{current}" step="0.05" jump="0.5" onChange="{onChange}" />
        <Label text="{current}" />
    </Menu>
]]
...
demoUI.open = function() 
    local xmlData = xmlreader.parse(sliderXml)
    ...
    local boundData = databind:__wrap{current=4}
    boundData.onChange = function(newValue) boundData.current=newValue end
