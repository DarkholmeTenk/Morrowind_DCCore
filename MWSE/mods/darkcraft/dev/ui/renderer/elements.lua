...
local elements = {
    ...
    Slider = base.simpleElement:extend(function(parent, args) return parent:createSlider(args) end, 
            {current=eah.number, max=eah.number, step=eah.number, jump=eah.number}, {"PartScrollBar_changed"}):withWidget({"current"}),
    TextSelect = base.simpleElement:extend(function(parent, args) return parent:createTextSelect(args) end,
            {text=eah.str, state=eah.number}, {"mouseClick"}),
    FillBar = base.simpleElement:extend(function(parent, args) return parent:createFillBar(args) end,
            {current=eah.number, max=eah.number}):withWidget({"current", "max"})
}
return elements